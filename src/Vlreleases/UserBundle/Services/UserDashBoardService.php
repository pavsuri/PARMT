...
class UserDashBoardService {
    ...
    public function getUserHoursPerDate() {
        ...
        foreach ($hoursPerDate as $hoursdate) {
            $dayHours[] = array('date' => $hoursdate['date']->format('Y-m-d'),
                'hours' => round($hoursdate['duration']));
        }
        return json_encode($dayHours);
    }

    public function getUserTasksPerProject($resourceId, $projectId, $startDate = "", $endDate = "") {
    ...
    public function updateApprovals($allApprovals, $status) {
        ...
        $changeStatus = $userTaskRepository
                ->changeStatusOfApprovals($allApprovals, $status);
        return $changeStatus;
    }

    public function updateUserTaskEmail($idValues,$status) {

        $userTaskRepository = $this->em
                ->getRepository('VlreleasesUserBundle:UserTask');
        return $userTaskRepository->updateEmailUserTask($idValues,$status);
    }
    ...
    public function getProject($project) {
        $projectRepository = $this->em
                ->getRepository('VlreleasesUserBundle:Project');
    ...
    public function getActivity($activity) {
        $activityRepository = $this->em
                ->getRepository('VlreleasesUserBundle:Activity');
    ...
    public function getUser($userId) {
        $userRepository = $this->em
                ->getRepository('VlreleasesUserBundle:User');
